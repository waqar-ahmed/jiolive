...
class Api_v130_model extends CI_Model {
    ...
    *****/
    ...
    public function content_by_country_id($country_id='',$page=''){
        ...
        $this->db->where("find_in_set(".$country_id.",country_id) >",0);
        $this->db->where('publish', '1');
        $this->db->order_by("live_tv_id","DESC");
        $latest_movies  =   $this->db->get('live_tv')->result_array();
        $i              =   0;
        foreach ($latest_movies as $video):
            $response[$i]['videos_id']                  = $video['live_tv_id'];
           ...
           /* if($response[$i]['release'] !='' && $response[$i]['release'] !=NULL)
                $response[$i]['release']                    = date("Y",strtotime($video['release']));*/
            $response[$i]['runtime']                    = '';
            $response[$i]['is_paid']                    = '0';
            $response[$i]['is_tvseries']                = '0';
            /*if($video['is_tvseries'] =='1')
                $response[$i]['is_tvseries']            = '1';*/
            $response[$i]['video_quality']              = $video['stream_label'];
            $response[$i]['thumbnail_url']              = $this->common_model->get_tv_thumbnail($video['thumbnail']);
            $response[$i]['poster_url']                 = $this->common_model->get_tv_poster($video['poster']);
            $i++;
        endforeach;
...
//        else
       if($tv->stream_from =='m3u8' || $tv->stream_from =='hls'):
        ...
        $response['program_guide']                  = $this->get_all_program_guide_by_tv_td($tv->live_tv_id);    
        return $response;
    }
